...
export default function Recorder({ onResult }: RecorderProps) {
  ...
  async function handleStop() {
    ...
    if (!speech.transcript || speech.transcript.trim().length < 30) {
      setError('Too short. Speak for at least 15 seconds.')
      return
    }
    ...
    try {
      ...
      if (user) {
        await supabase.from('sessions').insert({
          ...
          pitch_data: pitchSummary,
          filler_words: analysis.filler_words,
          pronunciation_flags: analysis.pronunciation_flags,
          biggest_issue: analysis.biggest_issue,
  ...
  return (
    <div style={{ maxWidth: 400, margin: '0 auto', padding: '2rem 1rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {LANGS.map(l => (
            <button
              key={l.code}
              onClick={() => !recording && setLang(l.code)}
              style={{
      ...
      <div style={{ textAlign: 'center', padding: '2rem 0' }}>
        ...
        >
          {analyzing
            ? <div style={{ width: 20, height: 20, border: '2px solid #ccc', borderTopColor: '#666', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
            : recording
              ? <div style={{ width: 20, height: 20, background: '#A32D2D', borderRadius: 3 }} />
              : (
      ...
      {speech.transcript && (
        <div style={{ background: '#f7f7f5', borderRadius: 8, padding: '10px 12px', marginBottom: '1rem' }}>
          ...
          <p style={{ fontSize: 13, margin: 0, lineHeight: 1.6, color: '#333' }}>
            {speech.transcript.slice(-200)}{speech.transcript.length > 200 ? '...' : ''}
          </p>
        </div>
      )}
