...
function hzToNote(hz) {
  ...
  return { note: NOTES[((semi % 12) + 12) % 12], octave: Math.floor(semi / 12) + 4, hz: Math.round(hz) }
}
...
function autoCorrelate(buf, sr) {
  ...
  for (let i = 0; i < b.length; i++) for (let j = 0; j < b.length; j++) c[i + j] += b[i] * b[j]
  const d = c.slice(b.length)
  let pd = 0
  while (pd < d.length && d[pd] > d[pd + 1]) pd++
  let maxval = -1, maxpos = -1
  for (let i = pd; i < d.length; i++) { if (d[i] > maxval) { maxval = d[i]; maxpos = i } }
  let T0 = maxpos
